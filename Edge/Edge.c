...
int main(int argc, char* argv[]) {
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++) {
			...
			if (i == 0 || j == 0 || i == width - 1 || j == height - 1) {
				...
				Gy = (c[0] * Y1[(j - 1) * width + i] + c[2] * Y1[(j + 1) * width + i]) / (c[0] * (j - 1) + c[2] * (j + 1));
				Gx = (c[0] * Y1[j * width + (i - 1)] + c[2] * Y1[j * width + (i + 1)]) / (c[0] * (i - 1) + c[2] * (i + 1));
				G = sqrt(Gx * Gx + Gy * Gy);
	...
	FILE* outputFile = fopen("AICenterY_Edge.bmp", "wb");
